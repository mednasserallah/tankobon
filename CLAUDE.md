...
3. **Confidence flagging.** ML Kit's per-line confidence is captured on `DetectedTextLine.confidence` (a merged line takes the **worst** member's, via `min`). Lines below **`LOW_CONFIDENCE_THRESHOLD = 0.5f`** (in `DetectedTextLine.kt`, tunable; 0.5 chosen because clean lettering scores ~0.9+ while smudged/small/stylised text drops below it) are flagged with a ⚠️ warning icon + dimmed text — a hint to edit, never a block on copy/translate. The flag clears once the line is edited (`item.text != item.line.text`). Verified: a stray low-confidence "K" detection showed the flag.
