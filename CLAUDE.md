...
**STATUS: 🚧 IN PROGRESS on `feature/text-detection-translation`** (off `develop`). Goal: tap a reader button to detect all English text on the current page (on-device, offline, free), list the lines in manga reading order with per-line copy, and optionally translate each to Arabic. Detect → list → copy/translate only — **no inline overlay/editing on the image**.
